...
| ロール | 表示項目 |
|--------|---------|
| `member` | 未提出（下書き）の日報件数、差戻しの日報件数、直近7日の自分の日報一覧、**直近に運転した車両**、全社共有された新着ヒヤリ3件 |
...
- ヘッダー行は日本語の項目名とする
- 出力上限は 50,000 件。超過時は出力せず、一覧に戻して絞り込みを促すメッセージを表示する
- 出力できるのは運行管理者以上。運行管理者のCSVには自拠点の行だけが入る（全社共有された他拠点の事故・ヒヤリも出力対象にしない）
