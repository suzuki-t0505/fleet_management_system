...
| 検証の実装 | `Attachments` Context が件数・サイズ・形式を検証してから `Storage.put/3` を呼ぶ。件数の上限は既存の添付数を数えて判定する（`max_entries` だけでは通算を守れない） |
| 状態 | **`Storage.Gcs` は未検証**。開発環境にサービスアカウントの認証情報が無いため実行経路を確認していない。自動テストは `Storage.Local` に対して書いている。本番設定を入れる際に疎通確認が必要 |
