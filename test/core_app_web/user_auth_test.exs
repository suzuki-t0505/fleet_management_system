defmodule CoreAppWeb.UserAuthTest do
  use CoreAppWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias CoreApp.Accounts
  alias CoreApp.Accounts.Scope
  alias CoreAppWeb.UserAuth

  import CoreApp.AccountsFixtures

  @remember_me_cookie "_core_app_web_user_remember_me"
  @remember_me_cookie_max_age 60 * 60 * 24 * 14

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, CoreAppWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_user/3" do
    ...
    test "ログイン済みの利用者もダッシュボードに遷移する", %{conn: conn, user: user} do
      conn =
        conn
        |> assign(:current_scope, Scope.for_user(user))
        |> UserAuth.log_in_user(user)
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      CoreAppWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount :require_manager" do
    test "運行管理者はアクセスできる", %{conn: conn} do
      session = session_for(conn, manager_fixture())

      assert {:cont, socket} =
               UserAuth.on_mount(:require_manager, %{}, session, %LiveView.Socket{})

      assert socket.assigns.current_scope.role == :manager
    end

    test "管理者はアクセスできる", %{conn: conn} do
      session = session_for(conn, admin_fixture())

      assert {:cont, _socket} =
               UserAuth.on_mount(:require_manager, %{}, session, %LiveView.Socket{})
    end

    test "一般利用者はダッシュボードへ差し戻される", %{conn: conn} do
      session = session_for(conn, user_fixture())

      assert {:halt, socket} =
               UserAuth.on_mount(:require_manager, %{}, session, flash_socket())

      assert socket.assigns.flash["error"] == "このページを表示する権限がありません"
    end

    test "未ログインはログイン画面へ差し戻される", %{conn: conn} do
      session = get_session(conn)

      assert {:halt, socket} =
               UserAuth.on_mount(:require_manager, %{}, session, flash_socket())

      assert socket.assigns.flash["error"] == "ログインしてください"
    end
  end
  ...
  describe "on_mount :require_admin" do
    test "管理者はアクセスできる", %{conn: conn} do
      ...
      assert socket.assigns.current_scope.role == :admin
    end

    test "運行管理者はアクセスできない", %{conn: conn} do
      session = session_for(conn, manager_fixture())

      assert {:halt, socket} =
               UserAuth.on_mount(:require_admin, %{}, session, flash_socket())
  ...
  defp flash_socket do
    %LiveView.Socket{endpoint: CoreAppWeb.Endpoint, assigns: %{__changed__: %{}, flash: %{}}}
  end

  defp session_for(conn, user) do
    token = Accounts.generate_user_session_token(user)

    conn |> put_session(:user_token, token) |> get_session()
  end
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        CoreAppWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
